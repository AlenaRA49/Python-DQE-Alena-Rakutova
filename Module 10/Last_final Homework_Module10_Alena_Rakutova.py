...
class UserInformationCollector:
    ...
    def __init__(self):
        ...
        self.news_notification_shown = False
        self.private_ad_notification_shown = False
        self.feedback_notification_shown = False

    def transfer_news(self, news_info):
        if len(news_info) >= 3:
            self.header = "News"
            self.news = news_info[0]
            city_date_time = news_info[1]
    ...
    def newsletterTransformation(self):
    ...
    def insert_into_database(self):
        ...
        if self.header == "News" and not self.news_notification_shown:
            # Check if a similar record exists based on news and city
            cursor.execute("SELECT COUNT(*) FROM News WHERE text = ? AND City = ?", (self.news, self.city))
            count = cursor.fetchone()[0]
            if count == 0:
                cursor.execute("INSERT INTO News (header, text, City, Date_time) VALUES (?, ?, ?, ?)",
                               (self.header, self.news, self.city, self.date_time))
            else:
                self.news_notification_shown = True
                print("News record already exists, skipping insertion.")

        elif self.header == "Private Ad" and not self.private_ad_notification_shown:
            # Check if a similar record exists based on ad text and date
            cursor.execute("SELECT COUNT(*) FROM PrivateAd WHERE text = ? AND date = ?", (self.ads, self.date))
            count = cursor.fetchone()[0]
            if count == 0:
                cursor.execute("INSERT INTO PrivateAd (header, text, date, days_left) VALUES (?, ?, ?, ?)",
                               (self.header, self.ads, self.date, self.days_left))
            else:
                self.private_ad_notification_shown = True
                print("Private Ad record already exists, skipping insertion.")

        elif self.header == "Feedback" and not self.feedback_notification_shown:
            # Check if a similar record exists based on feedback text and feedback_value
            cursor.execute("SELECT COUNT(*) FROM Feedback WHERE text = ? AND feedback_value = ?",
                           (self.feedback, self.feedback_value))
            count = cursor.fetchone()[0]
            if count == 0:
                cursor.execute("INSERT INTO Feedback (header, text, feedback_value) VALUES (?, ?, ?)",
                               (self.header, self.feedback, self.feedback_value))
            else:
                self.feedback_notification_shown = True
                print("Feedback record already exists, skipping insertion.")
...
def main():
    ...
    user_collector.newsletterTransformation()
